...
//头部与左侧下载图标
var oDownload_iphone = getByClassName(document,'a','iphone');
var oDownload_android = getByClassName(document,'a','android');
var oDownload_ipad = getByClassName(document,'a','ipad')[0];
//app下载弹窗
//oDownload_iphone.onclick = oDownload_android.onclick = oTip_iphone.onclick = oTip_android.onclick = Popup;
oDownload_app.onclick = oDownload_iphone[0].onclick = oDownload_iphone[1].onclick = oDownload_android[0].onclick = oDownload_android[1].onclick = oDownload_ipad.onclick = Popup;
...
Sethover(oDownload_iphone[0],oTip_iphone);
Sethover(oDownload_android[0],oTip_android);
...
var oPopHeader = getByClassName(oFullPop,'h3','full-pop-header')[0];

//oFullPop弹窗函数
function Popup(){
	oFullPop.style.cssText="display:block; margin-left: -294px;margin-top: -183px;position: fixed;left: 50%;top: 50%;";
    oUiMask.style.display = 'block'; 
	//弹窗拖拽
	Drag(oPopHeader,oFullPop);
}
oPopClose.onclick = function(){
    oFullPop.style.display = oUiMask.style.display = 'none';   
}
oUiMask.onmousedown = oFullPop.onmousedown = function(ev){
    var ev = ev || event;
    ev.cancelBubble = true;
}
