...
//作用：得到文件类型为doc,xls,ppt,pdf的所有文件
File.prototype.getDocXlsPptPdf = function() {
    var arr = [];
    for (var i in this.dataList) {
        if (this.dataList[i] && (this.dataList[i].type=='doc' || this.dataList[i].type=='xls' || this.dataList[i].type=='ppt' || this.dataList[i].type=='pdf')) {
            arr.push(this.dataList[i]);
        }
    }
    return arr;
